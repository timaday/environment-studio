...
public final class ProfileValidator {
    ...
    public ProfileResult validate(ReadyToPublish definition, Profile profile) {
        if (!definition.checked().logicalDigest().equals(profile.logicalDefinitionDigest())) return reject("INCOMPATIBLE_DEFINITION");
        if (!id(profile.id()) || profile.revision() == null || profile.revision().signum() <= 0 || profile.entities().isEmpty()) return reject("INVALID_PROFILE");
        Map<String, EntityType> types = new HashMap<>(); definition.checked().definition().logical().entityTypes().forEach(t -> types.put(t.id(), t));
        ...
        return new ProfileResult.StructurallyValid(new ProfileResult.Checked(result, ProfileEncoding.digest(result)));
    }
