...
final class PlanComposition {
    static Draft merge(ReadyToPublish definition,Draft existing,Content target,ProfileComposer.Draft proposal) {
        ...
        definition.checked().definition().logical().relations().forEach(relation->declarations.put(relation.id(),relation));
    ...
    private static TargetIntent.EntityDecision unresolvedDecision(ReadyToPublish definition,TargetIntent.Ref reference) {
        var type=definition.checked().definition().logical().entityTypes().stream().filter(candidate->candidate.id().equals(reference.type())).findFirst().orElseThrow(()->new PlanRefusal(PlanRefusal.Code.PROFILE_REFUSED));
        ...
        var references=new TreeMap<String,TargetIntent.ReferenceValue>();
        definition.checked().definition().logical().relations().stream().filter(relation->relation.kind()==RelationKind.REFERENCE && relation.fromType().equals(reference.type()))
