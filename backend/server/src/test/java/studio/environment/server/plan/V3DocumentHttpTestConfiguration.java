...
import studio.environment.server.planning.V3DocumentHttpWitnesses;
...
@TestConfiguration
public class V3DocumentHttpTestConfiguration {
    ...
    @Bean @Primary PlanRuntime mockV3DocumentHttpRuntime(HostedSessions sessions, WorkspaceRuntime storage) {
        ...
        var witness = new PlanPorts.PublishedDefinition(new NativeCommand.Reference(OBJECT, "2"),
                "explicit-http-mock-publication", new PlanDefinition.V3(V3DocumentHttpWitnesses.definition()), List.of());
        ...
        var port = new ObservationPort() {
            ...
            public Reservation reserveV3(V3Selection selected) {
                if (!selected.compiled().equals(V3DocumentHttpWitnesses.definition())) throw new AssertionError("MOCK_FOREIGN_DEFINITION");
                return new Reservation.Admitted(new Permit() {
                    public ObservationResult observe(TransientCredentials credentials, Cancellation cancelled) {
                        ...
                        return V3DocumentHttpWitnesses.observation();
                    }
                    public void close() {}
                });
            }
