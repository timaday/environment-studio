...
class V3PlanWorkflowHttpBoundaryTest {
    ...
    V3WorkflowHttpSocketClient.Response sequentialRequest(V3WorkflowHttpSocketClient client, String method, String path, String body, boolean token) throws Exception {
        V3WorkflowHttpTestConfiguration.awaitRecords(0);
    ...
    tools.jackson.databind.JsonNode bindings(V3WorkflowHttpSocketClient client,String plan,String revision,tools.jackson.databind.JsonNode entity)throws Exception{return ok(sequentialRequest(client,"POST",path(plan,"bindings"),JSON.writeValueAsString(Map.of("revision",revision,"entity",entity,"offset",0,"limit",100)),true));}
    ...
    tools.jackson.databind.JsonNode computed(V3WorkflowHttpSocketClient client,String plan,String revision,String side,String route,int offset,int limit)throws Exception{return ok(sequentialRequest(client,"POST",computedPath(plan,route),page(revision,side,offset,limit),true));}
    @Test void captureAndValidationUseTheActualOriginalXmlAndCompleteTargetRules()throws Exception {
        ...
        var physical=ok(sequentialRequest(client,"POST",path(plan,"entities"),page("2","current",0,100),true));
        ...
        var capture=ok(sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/profile-captures"," ".repeat(20000)+JSON.writeValueAsString(Map.of("revision","2","profileId","mock-profile","profileRevision","1","mappings",mappings)),true));
        ...
        var complete=ok(sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/validations",revision("2"),true));
        ...
        var first=ok(sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/validations",JSON.writeValueAsString(request),true));assertEquals(2,first.get("total").asInt());assertEquals(1,first.get("nextOffset").asInt());assertEquals(1,first.get("items").size());
        request.put("inputFingerprint",summary.get("inputFingerprint").asString());assertEquals(409,sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/validations",JSON.writeValueAsString(request),true).status());
    }
    ...
    @Test void previewReachesOwnedPublicationLookupAndReportsMissingProfile()throws Exception {
        ...
        var response=sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/profile-previews",JSON.writeValueAsString(request),true);
    ...
    @Test void wholeAndPartialCaptureSaveHistoryPreviewAndExplicitReuseRemainOwned()throws Exception {
        ...
        for(boolean whole:List.of(false,true)){
            ...
            var mappings=new ArrayList<Map<String,Object>>();int index=0;
            for(String identity:List.of("one","two","three"))mappings.add(Map.of("entity",item(physical,identity).get("entity"),"slotId","slot-"+(++index),"label","Neutral "+index));
            var capture=ok(sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/profile-captures",JSON.writeValueAsString(Map.of("revision","2","profileId","mock-profile","profileRevision","1","mappings",mappings)),true));
            ...
            var saved=ok(sequentialRequest(client,"PUT",profilePath,save,true));assertEquals(capture.get("source"),saved.get("source"));
            assertEquals(saved,ok(client.get(profilePath+"/revisions/1")));assertEquals(saved,ok(sequentialRequest(client,"PUT",profilePath,save,true)));
            ...
            var preview=ok(sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/profile-previews",JSON.writeValueAsString(previewRequest),true));
            ...
            previewRequest.put("section","dependencies");var dependencyPage=ok(sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/profile-previews",JSON.writeValueAsString(previewRequest),true));assertEquals(whole?0:1,dependencyPage.get("total").asInt());if(!whole){assertEquals("slot-2",dependencyPage.get("items").get(0).get("slotId").asString());assertEquals("declared-reuse-target",dependencyPage.get("items").get(0).get("reason").asString());}
            previewRequest.put("section","included");
            previewRequest.put("offset",50000);var beyond=ok(sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/profile-previews",JSON.writeValueAsString(previewRequest),true));assertTrue(beyond.get("items").isEmpty());assertEquals(preview.get("previewDigest"),beyond.get("previewDigest"));assertEquals(preview.get("affectedDerivations"),beyond.get("affectedDerivations"));
            ...
            var changed=ok(sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/commands",JSON.writeValueAsString(command),true));assertEquals("3",changed.get("revision").asString());assertEquals(changed,ok(sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/commands",JSON.writeValueAsString(command),true)));
            var unresolved=ok(sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/validations",revision("3"),true));assertFalse(unresolved.get("targetComplete").asBoolean());assertTrue(unresolved.get("computedRuleCount").isNull());
            ...
            ok(sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/commands",JSON.writeValueAsString(Map.of("expectedRevision","3","requestId",UUID.randomUUID().toString(),"kind","batch-upsert","changes",changes,"containment",List.of())),true));
            ...
            var validation=ok(sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/validations",revision("4"),true));assertTrue(validation.get("targetComplete").asBoolean());assertFalse(validation.get("exportAvailable").asBoolean());
    ...
    @Test void workflowOwnershipCsrfAndStalledBodyKeepTheOriginalScratch()throws Exception {
        ...
        for(String route:List.of("profile-captures","profile-previews","validations","reviews")){
            ...
            early(sequentialRequest(other,"POST",url,"{",true),404,"NOT_FOUND");assertEquals(403,sequentialRequest(client,"POST",url,revision("2"),false).status());assertEquals(400,sequentialRequest(client,"POST",url,"{",true).status());
        }
        V3WorkflowHttpTestConfiguration.awaitRecords(0);
        ...
        ok(sequentialRequest(other,"POST","/api/v3/plans/"+otherPlan+"/validations",revision("2"),true));early(login(subject).get("/api/v3/plans/current"),404,"NOT_FOUND");
    }
    ...
    void splitThroughReturnedProfileAndPlacement(V3WorkflowHttpSocketClient client,String plan,String profile,tools.jackson.databind.JsonNode original)throws Exception {
        var preview=ok(sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/profile-previews",JSON.writeValueAsString(Map.of("revision","4","profile",Map.of("objectId",profile,"workspaceRevision","1"),"selection",Map.of("kind","selected","roots",List.of("slot-1")),"section","included","offset",0,"limit",100)),true));
        ...
        var sheet=ok(sequentialRequest(client,"POST",path(plan,"placements"),JSON.writeValueAsString(Map.of("revision","5","documentId","sheet","projectionId","items","offset",0,"limit",100)),true)).get("items").get(0);
        var tail=ok(sequentialRequest(client,"POST",path(plan,"placements"),JSON.writeValueAsString(Map.of("revision","5","documentId","tail","projectionId","tail-items","offset",0,"limit",100)),true)).get("items").get(0);
        ...
        ok(sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/commands",JSON.writeValueAsString(Map.of("expectedRevision","5","requestId",UUID.randomUUID().toString(),"kind","batch-upsert","changes",changes,"containment",List.of())),true));
        ...
        var entities=ok(sequentialRequest(client,"POST",path(plan,"entities"),page("6","target",0,100),true));assertEquals(4,entities.get("total").asInt());assertEquals(item(original,"three").get("entity"),item(entities,"three").get("entity"));
        var locations=ok(sequentialRequest(client,"POST",path(plan,"binding-locations"),locations("6",fresh.get("split-a"),"tone","target",0,100),true));assertTrue(locations.get("total").asInt()>0);
        var contributors=ok(sequentialRequest(client,"POST",computedPath(plan,"contributors"),contributors("6","target",nodeSelector("gamma"),0,100),true));assertEquals(fresh.get("split-a"),contributors.get("items").get(0).get("physical"));assertEquals(digest(expectedSheet),contributors.get("items").get(0).get("origin").get("sourceDigest").asString());
