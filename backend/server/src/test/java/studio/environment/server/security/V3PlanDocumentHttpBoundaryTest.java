...
class V3PlanDocumentHttpBoundaryTest {
    ...
    tools.jackson.databind.JsonNode bindings(PlanHttpSocketClient client,String plan,String revision,tools.jackson.databind.JsonNode entity)throws Exception{return ok(client.request("POST",path(plan,"bindings"),JSON.writeValueAsString(Map.of("revision",revision,"entity",entity,"offset",0,"limit",100)),true));}
    ...
    @Test void actualMultiDocumentModesAndLocationsPreserveExactXmlAndQualifiedChildMappings() throws Exception{
        ...
        var entities=ok(client.request("POST",path(plan,"entities"),page("2","current",0,100),true));var one=item(entities,"one").get("entity");var two=item(entities,"two").get("entity");var oneBindings=bindings(client,plan,"2",one);var twoBindings=bindings(client,plan,"2",two);
    ...
    @Test void disclosureAndFreshLocationBoundariesApplyBeforeEmptyOrUnavailableResults() throws Exception{
        var owner=login("mock-document-boundary-"+UUID.randomUUID());String plan=create(owner);inspect(owner,plan);var entities=ok(owner.request("POST",path(plan,"entities"),page("2","current",0,100),true));var one=item(entities,"one").get("entity");var foreign=login("mock-document-foreign-"+UUID.randomUUID());
        for(String route:List.of("document","binding-locations")){early(foreign.request("POST",path(plan,route),"{",true),404,"NOT_FOUND");assertEquals(403,owner.request("POST",path(plan,route),"{}",false).status());assertEquals(400,owner.request("POST",path(plan,route),"{",true).status());}
        for(String mode:List.of("raw","placeholders","formatted")){
            assertEquals(422,owner.request("POST",path(plan,"document"),docBody("2","target","sheet",mode),true).status());
            for(Object disclosure:List.of(false,"true",1))assertEquals(400,owner.request("POST",path(plan,"document"),JSON.writeValueAsString(Map.of("revision","2","side","current","documentId","sheet","mode",mode,"completeDocumentDisclosure",disclosure)),true).status());
            assertEquals(400,owner.request("POST",path(plan,"document"),JSON.writeValueAsString(Map.of("revision","2","side","current","documentId","sheet","mode",mode)),true).status());
        }
        assertEquals(400,owner.request("POST",path(plan,"binding-locations"),JSON.writeValueAsString(Map.of("revision","2","entity",one,"fieldId","extra","side","current","offset",Integer.MAX_VALUE,"limit",1)),true).status());
        assertEquals(400,owner.request("POST",path(plan,"binding-locations"),JSON.writeValueAsString(Map.of("revision","2","entity",one,"fieldId","extra","side","current","offset",Integer.MAX_VALUE,"limit",1,"completeDocumentDisclosure",false)),true).status());
        var beyond=ok(owner.request("POST",path(plan,"binding-locations"),locations("2",one,"extra","current",Integer.MAX_VALUE,100),true));assertEquals(1,beyond.get("total").asInt());assertTrue(beyond.get("items").isEmpty());assertTrue(beyond.get("nextOffset").isNull());
        assertEquals(422,owner.request("POST",path(plan,"binding-locations"),locations("2",one,"extra","target",0,1),true).status());assertEquals(404,owner.request("POST",path(plan,"binding-locations"),locations("2",one,"unknown","current",0,1),true).status());
        assertEquals(409,owner.request("POST",path(plan,"document"),docBody("1","current","sheet","raw"),true).status());assertEquals(409,owner.request("POST",path(plan,"binding-locations"),locations("1",one,"id","current",0,1),true).status());
        var places=ok(owner.request("POST",path(plan,"placements"),JSON.writeValueAsString(Map.of("revision","2","documentId","sheet","projectionId","items","offset",0,"limit",100)),true));var coordinate=places.get("items").get(0);var fresh=Map.of("kind","fresh","slotId","fresh-child","typeId","item");var freshFields=new LinkedHashMap<String,Object>();for(String id:List.of("secret","optional"))freshFields.put(id,Map.of("kind","absent"));freshFields.put("id",Map.of("kind","entered","text","fresh"));freshFields.put("tone",Map.of("kind","entered","text","gamma"));freshFields.put("finish",Map.of("kind","entered","text","z"));freshFields.put("extra",Map.of("kind","entered","text","fresh-𐀀"));
