...
class V3PlanStructuralHttpBoundaryTest {
    ...
    @AfterEach void logoutAndCheckCanaries(CapturedOutput output) throws Exception {
        try { V3StructuralHttpTestConfiguration.awaitRecords(0); }
        finally {
            for (var client : clients) client.request("POST", "/api/v1/session/logout", "{}", true);
        }
        V3StructuralHttpTestConfiguration.awaitRecords(0);
    ...
    void command(PlanHttpSocketClient client,String plan,String expected,String next,Map<String,Object> action)throws Exception{
        ...
        var response=sequentialRequest(client,"POST","/api/v3/plans/"+plan+"/commands",JSON.writeValueAsString(wire),true);assertEquals(200,response.status());assertEquals(next,JSON.readTree(response.body()).get("revision").asString());
    }
    @Test void returnedPlacementsReferencesAndBindingsDriveExplicitStructuralWorkflow() throws Exception {
        ...
        var current=ok(sequentialRequest(client,"POST",path(plan,"entities"),page("2","current",0,100),true));var one=item(current,"one").get("entity");var two=item(current,"two").get("entity");var three=item(current,"three").get("entity");
        var relations=ok(sequentialRequest(client,"POST",path(plan,"relations"),page("2","current",0,100),true));assertEquals(1,relations.get("total").asInt());var relation=relations.get("items").get(0);assertEquals("link",relation.get("relationId").asString());assertEquals(one,relation.get("from"));assertEquals(two,relation.get("to"));
        var eligible=ok(sequentialRequest(client,"POST",path(plan,"placements"),placements("2","items"),true));assertEquals(1,eligible.get("total").asInt());var coordinate=eligible.get("items").get(0);assertEquals("sheet",coordinate.get("documentId").asString());assertEquals("0",coordinate.get("elementIndex").asString());assertEquals(digest(studio.environment.server.planning.V3StructuralHttpWitnesses.XML),coordinate.get("sourceDigest").asString());
        var before=ok(sequentialRequest(client,"POST",path(plan,"bindings"),bindings("2",two,0,100),true));var token=row(before,"id").get("token");assertEquals("two",row(before,"id").get("current").get("text").asString());
        ...
        var after=ok(sequentialRequest(client,"POST",path(plan,"bindings"),bindings("5",two,0,100),true));assertEquals(token,row(after,"id").get("token"));assertEquals("renamed",row(after,"id").get("target").get("text").asString());assertEquals(2,row(after,"id").get("targetLocations").get("total").asInt());
        var finalRelation=ok(sequentialRequest(client,"POST",path(plan,"relations"),page("5","target",0,100),true));assertEquals(relations.get("items"),finalRelation.get("items"));
        ...
        var freshBindings=ok(sequentialRequest(client,"POST",path(plan,"bindings"),bindings("6",fresh,0,100),true));assertEquals("unavailable",row(freshBindings,"id").get("current").get("state").asString());assertEquals("four",row(freshBindings,"id").get("target").get("text").asString());
        var target=ok(sequentialRequest(client,"POST",path(plan,"entities"),page("6","target",0,100),true));assertEquals(4,target.get("total").asInt());assertEquals(fresh,item(target,"four").get("entity"));
        var joined=JSON.createArrayNode();for(int offset=0;offset<4;offset++){var part=ok(sequentialRequest(client,"POST",path(plan,"entities"),page("6","target",offset,1),true));assertEquals(4,part.get("total").asInt());joined.add(part.get("items").get(0));}assertEquals(target.get("items"),joined);
        assertEquals(0,ok(sequentialRequest(client,"POST",path(plan,"containment"),draft("6",0,100),true)).get("total").asInt());
        command(client,plan,"6","7",Map.of("kind","upsert-entity","decision",Map.of("kind","remove","entity",three),"placements",List.of()));
        var all=ok(sequentialRequest(client,"POST",path(plan,"draft"),draft("7",0,100),true));assertEquals(4,all.get("total").asInt());boolean removed=false;for(var entry:all.get("items"))if(entry.get("entity").equals(three)){removed=true;assertEquals("remove",entry.get("disposition").asString());assertTrue(entry.get("fields").isEmpty());assertTrue(entry.get("references").isEmpty());assertTrue(entry.get("placements").isEmpty());}assertTrue(removed);
        ...
        var inventory=ok(sequentialRequest(client,"POST",path(plan,"documents"),revision("7"),true));assertEquals(digest(expected),inventory.get("documents").get(0).get("targetDigest").asString());
    }
    @Test void structuralRoutesRetainClosedRevisionAndOwnershipBoundaries() throws Exception {
        ...
        var entity=item(ok(sequentialRequest(owner,"POST",path(plan,"entities"),page("2","current",0,100),true)),"one").get("entity");
        for(String view:List.of("relations","draft","containment","placements","bindings")){
            early(sequentialRequest(foreign,"POST",path(plan,view),"{",true),404,"NOT_FOUND");assertEquals(403,sequentialRequest(owner,"POST",path(plan,view),"{}",false).status());assertEquals(400,sequentialRequest(owner,"POST",path(plan,view),"{",true).status());
            String stale=switch(view){case "relations"->page("1","current",0,100);case "placements"->placements("1","items");case "bindings"->bindings("1",entity,0,100);default->draft("1",0,100);};assertEquals(409,sequentialRequest(owner,"POST",path(plan,view),stale,true).status());
        }
        var beyond=ok(sequentialRequest(owner,"POST",path(plan,"bindings"),bindings("2",entity,256,100),true));assertEquals(5,beyond.get("total").asInt());assertTrue(beyond.get("items").isEmpty());assertTrue(beyond.get("nextOffset").isNull());assertEquals(400,sequentialRequest(owner,"POST",path(plan,"bindings"),bindings("2",entity,257,1),true).status());
        assertEquals(400,sequentialRequest(owner,"POST",path(plan,"draft"),"{\"revision\":\"2\",\"offset\":0,\"limit\":1,\"revealSecrets\":true}",true).status());
    ...
    @Test void heldBindingRequestPreservesSingleScratchAndCrossOwnerRecovery() throws Exception {
        String subject="mock-structural-held-"+UUID.randomUUID();var owner=login(subject);String plan=create(owner);inspect(owner,plan);var entity=item(ok(sequentialRequest(owner,"POST",path(plan,"entities"),page("2","current",0,100),true)),"one").get("entity");var other=login("mock-structural-waiter-"+UUID.randomUUID());String b=create(other);inspect(other,b);
        ...
        assertEquals(0,ok(sequentialRequest(other,"POST",path(b,"draft"),draft("2",0,100),true)).get("total").asInt());var renewed=login(subject);early(renewed.get("/api/v3/plans/current"),404,"NOT_FOUND");
    }
}
