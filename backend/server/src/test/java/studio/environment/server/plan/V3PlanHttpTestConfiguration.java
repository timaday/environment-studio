...
@TestConfiguration
public class V3PlanHttpTestConfiguration {
    ...
    @Bean @Primary PlanRuntime mockV3HttpRuntime(HostedSessions sessions, WorkspaceRuntime storage) {
        ...
        var witness = new PlanPorts.PublishedDefinition(new NativeCommand.Reference(OBJECT, "2"),
                "explicit-http-mock-publication", new PlanDefinition.V3(V3PlanHttpWitnesses.definition()), List.of());
        ...
        var port = new ObservationPort() {
            ...
            public Reservation reserveV3(V3Selection selected) {
                if (!selected.compiled().equals(V3PlanHttpWitnesses.definition())) throw new AssertionError("MOCK_FOREIGN_DEFINITION");
                return new Reservation.Admitted(new Permit() {
                    public ObservationResult observe(TransientCredentials credentials, Cancellation cancelled) {
                        ...
                        return V3PlanHttpWitnesses.observation();
                    }
                    public void close() {}
                });
            }
