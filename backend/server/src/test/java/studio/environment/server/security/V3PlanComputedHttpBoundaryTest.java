...
class V3PlanComputedHttpBoundaryTest {
    ...
    tools.jackson.databind.JsonNode computed(PlanHttpSocketClient client,String plan,String revision,String side,String route,int offset,int limit)throws Exception{return ok(client.request("POST",computedPath(plan,route),page(revision,side,offset,limit),true));}
    @Test void computedCollectionsPreserveCompleteOrderingAndActualContributorLocations()throws Exception{
        ...
        var physical=ok(client.request("POST",path(plan,"entities"),page("2","current",0,100),true));var one=item(physical,"one").get("entity");var two=item(physical,"two").get("entity");
        var alpha=ok(client.request("POST",computedPath(plan,"contributors"),contributors("2","current",nodeSelector("alpha"),0,100),true));assertEquals(2,alpha.get("total").asInt());assertEquals(one,alpha.get("items").get(0).get("physical"));assertEquals(two,alpha.get("items").get(1).get("physical"));
        ...
        var pair=Map.of("kind","cooccurrence","relation","pair","source",key("tones","by-tone","alpha"),"target",key("finishes","by-finish","x"));var evidence=ok(client.request("POST",computedPath(plan,"contributors"),contributors("2","current",pair,0,100),true));assertEquals(1,evidence.get("total").asInt());assertEquals(one,evidence.get("items").get(0).get("physical"));assertEquals("tone",evidence.get("items").get(0).get("roles").get(0).get("field").asString());assertEquals("finish",evidence.get("items").get(0).get("roles").get(1).get("field").asString());
    }
    @Test void targetEditsPreserveReferencesAndRemoveOnlyLastComputedContributor()throws Exception{
        var client=login("mock-computed-edits-"+UUID.randomUUID());String plan=create(client);inspect(client,plan);var physical=ok(client.request("POST",path(plan,"entities"),page("2","current",0,100),true));var one=item(physical,"one").get("entity");var three=item(physical,"three").get("entity");
        assertEquals(422,client.request("POST",computedPath(plan,"nodes"),page("2","target",0,100),true).status());
        ...
        var decision=Map.of("kind","retain","entity",one,"fields",fields,"references",Map.of("link",Map.of("kind","keep-observed")));var ack=client.request("POST","/api/v3/plans/"+plan+"/commands",JSON.writeValueAsString(Map.of("expectedRevision","2","requestId",UUID.randomUUID().toString(),"kind","upsert-entity","decision",decision,"placements",List.of())),true);assertEquals(200,ack.status());
        var beta=ok(client.request("POST",computedPath(plan,"contributors"),contributors("3","target",nodeSelector("beta"),0,100),true));assertEquals(2,beta.get("total").asInt());assertEquals(one,beta.get("items").get(0).get("physical"));var original=ok(client.request("POST",computedPath(plan,"contributors"),contributors("3","current",nodeSelector("alpha"),0,100),true));assertEquals(2,original.get("total").asInt());
        var pair=Map.of("kind","cooccurrence","relation","pair","source",key("tones","by-tone","beta"),"target",key("finishes","by-finish","x"));assertEquals(2,ok(client.request("POST",computedPath(plan,"contributors"),contributors("3","target",pair,0,100),true)).get("total").asInt());
        fields.put("id",Map.of("kind","entered","text","renamed"));assertEquals(200,client.request("POST","/api/v3/plans/"+plan+"/commands",JSON.writeValueAsString(Map.of("expectedRevision","3","requestId",UUID.randomUUID().toString(),"kind","upsert-entity","decision",decision,"placements",List.of())),true).status());
        assertEquals(one,ok(client.request("POST",computedPath(plan,"contributors"),contributors("4","target",nodeSelector("beta"),0,100),true)).get("items").get(0).get("physical"));
        int revision=4;for(var ref:List.of(one,three)){var removed=client.request("POST","/api/v3/plans/"+plan+"/commands",JSON.writeValueAsString(Map.of("expectedRevision",Integer.toString(revision),"requestId",UUID.randomUUID().toString(),"kind","upsert-entity","decision",Map.of("kind","remove","entity",ref),"placements",List.of())),true);assertEquals(200,removed.status());revision++;if(revision==5)assertEquals(1,ok(client.request("POST",computedPath(plan,"contributors"),contributors("5","target",nodeSelector("beta"),0,100),true)).get("total").asInt());}
        assertEquals(404,client.request("POST",computedPath(plan,"contributors"),contributors("6","target",nodeSelector("beta"),0,100),true).status());assertEquals(2,computed(client,plan,"6","target","nodes",0,100).get("total").asInt());assertEquals(4,computed(client,plan,"6","current","nodes",0,100).get("total").asInt());
    }
    @Test void computedConsentOwnershipAndHeldBodyRetainOriginalCapacity()throws Exception{
        ...
        for(String route:List.of("nodes","memberships","cooccurrences","rules","contributors")){String request=route.equals("contributors")?contributors("2","current",nodeSelector("alpha"),0,100):page("2","current",0,100);early(foreign.request("POST",computedPath(plan,route),"{",true),404,"NOT_FOUND");assertEquals(403,owner.request("POST",computedPath(plan,route),request,false).status());assertEquals(400,owner.request("POST",computedPath(plan,route),"{",true).status());assertEquals(409,owner.request("POST",computedPath(plan,route),request.replace("\"revision\":\"2\"","\"revision\":\"1\""),true).status());}
        var base=new LinkedHashMap<String,Object>(Map.of("revision","2","side","current","selector",nodeSelector("missing"),"offset",Integer.MAX_VALUE,"limit",1));for(Object consent:List.of(false,"true",1)){base.put("completeDocumentDisclosure",consent);assertEquals(400,owner.request("POST",computedPath(plan,"contributors"),JSON.writeValueAsString(base),true).status());}base.remove("completeDocumentDisclosure");assertEquals(400,owner.request("POST",computedPath(plan,"contributors"),JSON.writeValueAsString(base),true).status());
        var beyond=ok(owner.request("POST",computedPath(plan,"contributors"),contributors("2","current",nodeSelector("alpha"),Integer.MAX_VALUE,100),true));assertEquals(2,beyond.get("total").asInt());assertTrue(beyond.get("items").isEmpty());assertTrue(beyond.get("nextOffset").isNull());assertEquals(404,owner.request("POST",computedPath(plan,"contributors"),contributors("2","current",nodeSelector("x".repeat(20_000)),0,1),true).status());
