...
final class PlanViewEncoding extends OutputStream implements AutoCloseable {
    ...
    private final int maximum;private final List<byte[]> chunks=new ArrayList<>();private int size,used;private boolean attempted,complete,closed;
    PlanViewEncoding(int maximum){if(maximum<1 || maximum>134_217_728)throw new IllegalArgumentException("INVALID_RESPONSE_LIMIT");this.maximum=maximum;}
    void encode(Object value){
        if(attempted || closed)throw new IllegalStateException("RESPONSE_ENCODING_CONSUMED");attempted=true;
        try {JSON.writeValue(new FilterOutputStream(this){@Override public void close(){} @Override public void write(byte[] bytes,int offset,int length){PlanViewEncoding.this.write(bytes,offset,length);}},value);complete=true;}
    ...
    @Override public void write(int value){if(closed || size>=maximum)throw new PlanRefusal(PlanRefusal.Code.RESOURCE_LIMIT);space();chunks.getLast()[used++]=(byte)value;size++;}
    @Override public void write(byte[] bytes,int offset,int length){
        if(closed || length>maximum-size)throw new PlanRefusal(PlanRefusal.Code.RESOURCE_LIMIT);
        while(length>0){space();int count=Math.min(length,chunks.getLast().length-used);System.arraycopy(bytes,offset,chunks.getLast(),used,count);offset+=count;length-=count;used+=count;size+=count;}
    }
    private void space(){if(chunks.isEmpty() || used==chunks.getLast().length){chunks.add(new byte[Math.min(8192,maximum-size)]);used=0;}}
