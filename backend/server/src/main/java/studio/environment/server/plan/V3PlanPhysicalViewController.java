...
public final class V3PlanPhysicalViewController {
    private final PlanRuntime runtime;
    private final HostedSessions sessions;
    public V3PlanPhysicalViewController(PlanRuntime runtime, HostedSessions sessions) {
        this.runtime = runtime; this.sessions = sessions;
    }
    @PostMapping("/api/v3/plans/{planId}/views/documents")
    public void documents(@PathVariable("planId") String planId, HttpServletRequest request, HttpServletResponse response) {
        start(planId, request, response, V3PlanTransport.PhysicalRoute.DOCUMENTS);
    }
    @PostMapping("/api/v3/plans/{planId}/views/entities")
    public void entities(@PathVariable("planId") String planId, HttpServletRequest request, HttpServletResponse response) {
        start(planId, request, response, V3PlanTransport.PhysicalRoute.ENTITIES);
    }
