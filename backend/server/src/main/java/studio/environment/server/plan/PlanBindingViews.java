...
final class PlanBindingViews {
    static Map<String,Object> bindings(HostedPlanService.ViewSnapshot snapshot,PlanCommand.Ref ref,int offset,int limit){
        bounds(offset,limit,256);var entity=PlanBindings.resolve(snapshot,ref);
        boolean fresh=entity.reference() instanceof TargetIntent.Ref.Fresh;
        var current=fresh?Map.<String,Integer>of():counts(snapshot,false,entity.reference());
        var target=snapshot.target().isPresent()?counts(snapshot,true,entity.reference()):Map.<String,Integer>of();
        ...
        for(var field:entity.fields().subList(Math.min(offset,total),end)) {
            var currentCount=fresh?unavailable("CURRENT_ENTITY_ABSENT"):complete(current.getOrDefault(field.fieldId(),0));
    ...
    static Map<String,Object> locations(HostedPlanService.ViewSnapshot snapshot,PlanCommand.Ref ref,String field,boolean target,int offset,int limit,boolean disclosed){
        ...
        bounds(offset,limit,Integer.MAX_VALUE);var entity=PlanBindings.resolve(snapshot,ref);
        ...
        PlanBindingLocations.scan(snapshot,target,occurrence->{if(occurrence.entity().equals(entity.reference()) && occurrence.fieldId().equals(field))page.add(occurrence.location());});
    ...
    private static Map<String,Integer> counts(HostedPlanService.ViewSnapshot snapshot,boolean target,TargetIntent.Ref entity){
