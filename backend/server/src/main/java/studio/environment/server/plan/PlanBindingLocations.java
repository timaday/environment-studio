...
final class PlanBindingLocations {
    ...
    static void scan(HostedPlanService.ViewSnapshot snapshot,boolean target,Consumer<Occurrence> consumer) {
        ...
        var binding=snapshot.definition().compiled().checked().definition().bindings().stream().filter(b->b.id().equals(snapshot.binding())).findFirst().orElseThrow(PlanBindingLocations::refused);
        var documents=new HashMap<String,NativeDefinition.Document>();binding.documents().forEach(d->documents.put(d.id(),d));
        var types=new HashMap<String,NativeDefinition.EntityType>();snapshot.definition().compiled().checked().definition().logical().entityTypes().forEach(t->types.put(t.id(),t));
        var referenceRelations=new HashSet<String>();
        snapshot.definition().compiled().checked().definition().logical().relations().stream().filter(r->r.kind()==studio.environment.core.definition.DefinitionDraft.RelationKind.REFERENCE).forEach(r->referenceRelations.add(r.id()));
