...
class CapabilitiesController {
    ...
    CapabilitiesController(studio.environment.server.security.RuntimeConfiguration.RuntimeMode mode, studio.environment.server.workspace.WorkspaceRuntime workspace) { this.mode = mode; this.workspace = workspace; }
    record Capabilities(String mode, boolean definitionWorkspaceEnabled, boolean inspectionEnabled, boolean exportEnabled,
                        List<String> qualifiedDatabaseAdapters, List<String> blockers) { }

    @GetMapping("/api/v1/capabilities")
    Capabilities capabilities() {
        return new Capabilities(mode.name().toLowerCase(java.util.Locale.ROOT), workspace.enabled(), false, false, List.of(),
