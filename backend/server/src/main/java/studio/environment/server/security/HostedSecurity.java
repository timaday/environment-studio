...
public class HostedSecurity {
    ...
    @Bean @ConditionalOnProperty(name="studio.security.local-operator.enabled", havingValue="false", matchIfMissing=true)
    ClientRegistrationRepository registrations(HostedSettings settings) {
    ...
    @Bean @ConditionalOnProperty(name="studio.security.local-operator.enabled", havingValue="false", matchIfMissing=true)
    SecurityFilterChain hostedFilterChain(HttpSecurity http, HostedSettings settings,
        ...
        http.authorizeHttpRequests(HostedSecurity::hostedRoutes);
    ...
    @Bean @ConditionalOnProperty(name="studio.security.local-operator.enabled", havingValue="true")
    LocalOperatorSettings localOperatorSettings(Environment environment) { return new LocalOperatorSettings(environment); }
    @Bean @ConditionalOnProperty(name="studio.security.local-operator.enabled", havingValue="true")
    SecurityFilterChain localHostedFilterChain(HttpSecurity http, HostedSettings settings,
            HostedSessions sessions, LocalOperatorSettings local) throws Exception {
        http.addFilterBefore(new LocalOperatorAuthenticationFilter(settings, sessions, local), CsrfFilter.class);
        http.addFilterAfter(new HostedBoundaryFilter(settings, sessions), LocalOperatorAuthenticationFilter.class);
        http.authorizeHttpRequests(HostedSecurity::hostedRoutes);
        http.requestCache(cache -> cache.disable());
        http.exceptionHandling(errors -> errors
                .authenticationEntryPoint((request, response, exception) -> {
                    response.setHeader("WWW-Authenticate", "Basic realm=\"Environment Studio\", charset=\"UTF-8\"");
                    SafeResponses.refuse(response, 401, "AUTHENTICATION_REQUIRED");
                })
                .accessDeniedHandler((request, response, exception) -> SafeResponses.refuse(response, 403, "REQUEST_DENIED")));
        http.sessionManagement(session -> session.sessionFixation(fixation -> fixation.changeSessionId()));
    ...
    private static void hostedRoutes(AuthorizeHttpRequestsConfigurer<HttpSecurity>.AuthorizationManagerRequestMatcherRegistry auth) {
        auth.requestMatchers(HttpMethod.GET, "/", "/index.html", "/assets/**", "/brand/**", "/api/v1/capabilities",
                ...
                .requestMatchers(HttpMethod.POST, "/api/v3/plans", "/api/v3/plans/{planId}/inspections", "/api/v3/plans/{planId}/commands", "/api/v3/plans/{planId}/materializations",
                        ...
                        "/api/v3/plans/{planId}/reviews", "/api/v3/plans/{planId}/package-candidates/guarded",
                ...
                .requestMatchers(HttpMethod.POST, "/api/v1/plans/{planId}/materializations",
                        ...
                        "/api/v1/plans/{planId}/views/draft", "/api/v1/plans/{planId}/views/containment", "/api/v1/plans/{planId}/views/placements",
                        "/api/v1/plans/{planId}/views/document", "/api/v1/plans/{planId}/views/bindings", "/api/v1/plans/{planId}/views/binding-locations",
                        "/api/v1/plans/{planId}/profile-captures", "/api/v1/plans/{planId}/profile-previews", "/api/v1/plans/{planId}/validations").authenticated()
