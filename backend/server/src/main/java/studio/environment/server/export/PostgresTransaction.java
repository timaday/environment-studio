...
final class PostgresTransaction {
    ...
    private static void write(PackageAdmission.Result.Accepted input, Source s) {
        ...
        guard(s,"current_setting('server_version_num') <> '"+(version16?"160011":"180006")+"' OR current_setting('server_encoding') <> 'UTF8' OR current_setting('client_encoding') <> 'UTF8' OR current_setting('transaction_read_only') <> 'off'","ES_SETTINGS_MISMATCH");
        ...
        guard(s,"EXISTS(SELECT 1 FROM pg_catalog.pg_inherits WHERE inhrelid=es_oid OR inhparent=es_oid) OR EXISTS(SELECT 1 FROM pg_catalog.pg_policy WHERE polrelid=es_oid) OR EXISTS(SELECT 1 FROM pg_catalog.pg_rewrite WHERE ev_class=es_oid)","ES_WRITE_EFFECT_UNSUPPORTED");
        ...
        guard(s,"EXISTS(SELECT 1 FROM pg_catalog.pg_constraint c WHERE (c.conrelid=es_oid OR c.confrelid=es_oid) AND (c.contype NOT IN ("+(version16?"'p','u'":"'p','u','n'")+") OR c.condeferrable OR NOT c.convalidated"+(version16?"":" OR NOT c.conenforced")+"))","ES_CONSTRAINT_UNSUPPORTED");
