...
/** Read-only v3 publication lookup. No runtime or HTTP composition is installed. */
public final class V3PlanWorkspaceBridge implements V3PlanWorkspace {
    ...
    private V3NativeRevision read(Owner owner, NativeCommand.Reference reference, boolean profile) {
        var revision = store.read(owner, reference.objectId(), Optional.of(reference.workspaceRevision()), profile);
