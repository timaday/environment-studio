...
class Assemble {
    public static void main(String[] args)throws Exception {
        ...
        String launcher="#!/bin/sh\nset -eu\nulimit -c 0\nbase=$(CDPATH= cd -- \"${0%/*}\" && pwd -P)\nexec /usr/bin/env -i LANG=C.UTF-8 LC_ALL=C.UTF-8 /usr/bin/java -XX:ErrorFile=/dev/null -XX:-CreateCoredumpOnCrash -XX:-HeapDumpOnOutOfMemoryError -cp \""+cp+"\" studio.environment.supervisor.Main \"$@\"\n";
