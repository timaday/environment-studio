...
public final class CapacityAssemblyShapeProbe {
    ...
    CapacityAssemblyShapeProbe() throws Exception {
    ...
    void payload(Plan plan) {
        try (var view = service.reserveView(plan.lease(), plan.id(), PlanDefinition.Version.V3)) {
            view.run(() -> {
                ...
                var accepted = admit(actual, plan, validation.inputFingerprint(), view.snapshot().definition().publicationDigest());
                stage("PACKAGE_ASSEMBLY_BEGIN");
                var digest = digest();
                long[] written = new long[]{0};
                var assembled = new studio.environment.server.export.GuardedPackageAssembler().write(accepted, new OutputStream() {
                    @Override public void write(int value) {
                        digest.update((byte)value); written[0]++;
                    }
                    @Override public void write(byte[] bytes, int offset, int length) {
                        digest.update(bytes, offset, length); written[0] += length;
                    }
                }, new ObservationPort.Cancellation());
                check(assembled instanceof studio.environment.server.export.GuardedPackageAssembler.Result.Candidate, "PACKAGE_ASSEMBLY_CANDIDATE_REQUIRED");
                var candidate = (studio.environment.server.export.GuardedPackageAssembler.Result.Candidate) assembled;
                check(!candidate.qualified(), "PACKAGE_ASSEMBLY_NOT_QUALIFIED");
                equal(written[0], candidate.bytes(), "PACKAGE_ASSEMBLY_SIZE");
                equal(java.util.HexFormat.of().formatHex(digest.digest()), candidate.sha256(), "PACKAGE_ASSEMBLY_DIGEST");
                stage("PACKAGE_ASSEMBLY_VERIFIED");
                view.verify(); stage("PAYLOAD_VERIFIED");
                System.out.println("PAYLOAD_BYTES="+actual.length+" ARCHIVE_BYTES="+candidate.bytes()+" qualified=false exactCanonical=true");
                return true;
            });
        }
    }
    ...
    studio.environment.server.export.PackageAdmission.Result.Accepted admit(byte[] payload,Plan plan,String fingerprint,String publicationDigest) {
        try {
    ...
    void run() throws Exception {
        ...
        System.out.println("ASSEMBLY_SHAPE_PASS retainedOwners=4 recoveredSameOwner=NOT_RUN documentsPerOwner="+SHAPE.documents+" physicalPerOwner="+SHAPE.physical+" rulesPerOwner="+(SHAPE.groups*32)+" currentTarget=exact heldLogout=NOT_RUN publication=MOCK compiler=INCOMPLETE wirePaging=NOT_RUN");
    }
    public static void main(String[] args) throws Exception { new CapacityAssemblyShapeProbe().run(); }
}
