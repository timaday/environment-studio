...
active XInclude and XML 1.1 return safe refusals. XML Signature and XML Encryption elements are preserved as opaque content when not selected by definitions. Inert top-level DOCTYPE declarations are preserved in source/target text and stripped only from the parser projection copy. Full-target
