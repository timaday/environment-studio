Latest lead checkpoint — 13 September 2026: Export view copy was tightened after Tim approved the view direction with ‘less is more where possible’. No API or authority behavior changed: package download still requires a PASS backend validation summary for the exact revision and the contracted unqualified ZIP response, and the application still exposes no SQL execution control. Verification: `npm test --prefix frontend -- V3ExportJourney.test.tsx` PASS through 455 Vitest tests and 61 schema contract tests; `npm run check --prefix frontend` PASS; pinned frontend build PASS; pinned Maven server `process-resources` PASS; hosted `profiles-v3-values` desktop/narrow browser PASS for Values → Validation → Export blocked path; repository content/integrity/diff/script guards PASS. One RED browser run is recorded as stale packaged-asset evidence before rebuilding resources; the corrected packaged asset passed afterward.
