Ready: PostgreSQL 16.11 guarded package candidate route is implemented and published on `implementation/v3-postgres16-guarded-package-route-20260912`; current package-route handoff is in issue #9 at `93018c14d048fb6856916db544f9067cb0903806`. Focused package, admission, template, destination-config and HTTP download checks pass 45 tests; OpenAPI contract route/schema checks pass; standard repository/content checks, full backend Maven verify passes 1,735 module-total tests on the witness line, and exact-source local G08 image smoke passes for the route line. A local opt-in PostgreSQL 16.11 supervisor/client witness now proves the invented C/UTF-8 disposable database path for commit acknowledgement, pre-program rollback acknowledgement and post-COMMIT unknown classification. Full MVP/release qualification remains open; today's viable deliverable is the unqualified guarded package download route plus local supervisor/client witness for the reviewed mock workflow.

# Handoff and capability status

## Current lead checkpoint — 12 September 2026, 15:58 UTC

Lead owns application/native/integration; reviewer is re-onboarded through issue #9 for read-only review of immutable candidates; IDE2 remains retired. Current worktree `/home/tim/.tmp/es-v3-package-read-resource-20260911`, branch `implementation/v3-postgres16-guarded-package-route-20260912`, base `f28e30f35aace1b3107e67d1d8d84f5b705caae1`; exact published heads are tracked in issue #9. The candidate adds the hosted unqualified V3 guarded package download route for configured PostgreSQL 16.11/psql 16.11/linux-amd64 targets. It rechecks plan ownership, pinned revision, inspection, complete target, validation fingerprint, destination identity, protected document policies and package admission before streaming a deterministic ZIP; it keeps `exportAvailable=false` and does not execute SQL or persist packages. Focused author run passes 45 tests; standard repository/content checks pass; full backend Maven verify passes 1,735 module-total tests on the witness line; exact-source local runtime image `sha256:d55e30a9c1dc799f6f02c418281122d6525ad1273f410a34a867254efeace3b2` passes G08 build/smoke/workspace checks and supervisor artifact checksum export; [evidence](../evidence/v3-postgres16-guarded-package-route.md) and [operator handoff](../evidence/postgresql-1611-operator-handoff.md).

Ready: publish the local PostgreSQL 16.11 supervisor/client witness amendment, then continue toward deployable operator handoff. Awaiting external evidence: reviewer disposition on the exact published candidate, real production definition publication evidence and production native/client execution qualification. Genuine blockers for full MVP today remain production supervisor/client/TLS/commit qualification, full hosted release evidence and remaining operator UI/release journeys. TEST-QA-010/011 and ASM-QA-C01 are accepted and locally corrected/reconciled after the route commit; [evidence](../evidence/package-capacity-probe-settlement.md).
