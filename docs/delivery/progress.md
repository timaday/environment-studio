Ready: reviewer findings TEST-QA-013 and TEST-QA-014 are accepted and corrected locally on `implementation/v3-postgres16-guarded-package-route-20260912`. Hosted replay synchronization now waits for command scratch settlement before asserting the sequential stale replay conflict returns 409, preserving legal contention 429 behavior while the original worker is still closing. PostgreSQL witness cleanup now requires exact Docker absence diagnostics for the owned container and owned anonymous volumes; daemon/permission/transport failures are rejected as inconclusive, not counted as absence. Focused checks pass: `HostedBoundaryTest#actualHttpOneToTwoCrossDocumentTargetReplayForeignBodyAndFailedInspection`, `Postgres16ClientWitnessCleanupOracleTest`, default `Postgres16ClientWitnessTest` with 3 skipped, and explicit `ES_POSTGRES16_CLIENT_WITNESS=true ES_POSTGRES16_IMAGE=postgres:16.11-bookworm Postgres16ClientWitnessTest` with 3 tests. Repository/content/script checks pass. Full backend Maven verify passes with core 335, qualified XML parser 7, server 1,049 and guarded-supervisor 350 tests, with the three Docker-gated PostgreSQL witness cases skipped by default. No production export/release qualification is claimed.

# Handoff and capability status

## Current lead checkpoint — 12 September 2026 20:20 Europe/London

Lead owns application/native/integration; reviewer is read-only through issue #9. Worktree `/home/tim/.tmp/es-v3-package-read-resource-20260911`, branch `implementation/v3-postgres16-guarded-package-route-20260912`, base `f28e30f35aace1b3107e67d1d8d84f5b705caae1`; parent pushed head before these corrections was `2e282dbd2598a6604313441c6640ff9bd09ec54c`. Current local changes are the accepted TEST-QA-013/014 corrections above. Focused checks, repository/content/script checks and full backend Maven verify pass. Next commands: commit, push, update issue #9 and PR #10, then continue only after review disposition or another independent ready slice.
