...
the first snapshot SELECT. The PostgreSQL 16.11 text pilot reads only rows where
the declared XML text column is non-null and non-empty; this fixed eligibility
predicate is generated by the adapter and is not definition-authored SQL. Read
identity, metadata and complete eligible inventory in that snapshot while holding
the lock; release it only at rollback. Require effective
...
fidelity, complete eligible membership and
concurrent changes across one snapshot. Challenge missing/extra eligible rows,
denied metadata/SELECT, RLS/VPD-hidden rows, role/column write grants, wrong
destination, extra PostgreSQL null/empty XML rows, null/empty declared-source
rows, multibyte CLOB streaming, exact size boundaries, stalled reads,
